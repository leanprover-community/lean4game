...
This instructions aims to give you an examplatory manual on how to host your
own lean4game instance, such that, others can access it via the internet.
...
In this section we will use nginx for our server but any other webserver might be applied here.
...
The next step would include seting up a reverse proxyy with nginx and adding
informtion concerning domain and SSL certification. As those considerations are
quite individual, technical and safety critical, we will refer here to a general instruction on how
to setup a reverse [proxy using nginx on Ubunutu](https://www.digitalocean.com/community/tutorials/how-to-configure-nginx-as-a-reverse-proxy-on-ubuntu-22-04).

## Setting up Lean4Game

This section will be somwhat similar to what is written in the section about
[running Lean4Game locally](doc/running_locally.md).
...
To get the GitHub AccessToken required for fields ```LEAN4GAME_GITHUB_USER``` and ```LEAN4GAME_GITHUB_TOKEN``` please refer to the dedicated [instructions](./Updating/updating_github_access_token.md).
...
Having done that you should be able to now access your hosten Lean4Game instance via the domain that has been specified during the setup of the nginx server. For more information on how to maintain your established server please refer to the [server maintainance documentation](server.md)
