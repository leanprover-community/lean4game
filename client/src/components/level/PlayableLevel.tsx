...
import { codeAtom, editorConnectionAtom, leanMonacoAtom, leanMonacoEditorAtom, lockEditorModeAtom, proofAtom, typewriterModeAtom } from "../../store/editor-atoms"
...
import { EventEmitter } from "lean4monaco/dist/vscode-lean4/lean4-infoview/src/infoview/event"
import { EditorConnection, EditorEvents } from "lean4monaco/dist/vscode-lean4/lean4-infoview/src/infoview/editorConnection"
...
export function PlayableLevel() {
  ...
  // Keep as useState<>() because leanMonacoEditor is local to this component
  ...
  const [editorConnection, setEditorConnection] = useAtom(editorConnectionAtom)
  const [, setMonacoEditor] = useAtom(leanMonacoEditorAtom)

  // REPLACES EditorContext
  // Start the editor
  useEffect(() => {
    if (leanMonaco) {
      ...
      ;(async () => {
        ...
        // Set atom so other components can access it
        setMonacoEditor(leanMonacoEditor.editor ?? null)
      ...
      const infoviewApi: InfoviewApi = {
        ...
        clickedContextMenu: async () => console.log("clickedContextMenu")
      }
...
export function PlayableLevelIGNORE() {
  let { t } = useTranslation()
  const { t : gT } = useGameTranslation()
  const codeviewRef = useRef<HTMLDivElement>(null)
  const [leanMonaco] = useAtom(leanMonacoAtom)
  ...
  useEffect(() => {
    if (leanMonaco) {
      ...
      const editorConnection = new EditorConnection(infoProvider.editorApi, editorEvents)
      setEditorConnection(editorConnection)
      console.log("Editor Connection Atom has been initialized.")
